...
class AVL {
	...
	AVL();
	int Search(int x);
	int RInsert(int x, Node *D, Node **Dummy);
	int Insert(int x);
...
int AVL::RInsert(int x, Node *D, Node **RP) // D는 절대 NULL이아님
{
	Node *NN, *FC;
	int rv;
	if (D->a < x)
	...
	if (abs(D->LL - D->RL) > 1) // 차이가 2이상난 상황
	{
		...
		else if (x < D->a && x > LC->a) // case LR
		{
			...
			*RP = T1;
			if (T2 == NULL)
				...
				LC->RL = max(T2->LL, T2->RL) + 1;
			if (T3 == NULL)
				D->LL = 0;
			else
				D->LL = max(T3->LL, T3->RL) + 1;
		...
		else if (x > D->a && x < RC->a) // case RL
		{
			...
			*RP = T1;
			if (T2 == NULL)
				...
				D->RL = max(T2->LL, T2->RL) + 1;
			if (T3 == NULL)
				RC->LL = 0;
			else
				RC->LL = max(T3->LL, T3->RL) + 1;
	...
	return max(D->LL, D->RL); // 왼 오 레이블중 큰거를 리턴
}
